...
# --- DYNAMIC PATH RESOLUTION (Fixes FileNotFoundError issues) ---
# Resolve absolute paths dynamically relative to this script's location (src/agent.py)
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
PROJECT_ROOT = os.path.join(SCRIPT_DIR, '..')

# Define professional directory structure paths
CONFIG_PATH = os.path.join(PROJECT_ROOT, 'config', 'hyperparameters.yml')
...
class Agent():
    def __init__(self, hyperparameter_set):
         # Load configurations using the dynamic CONFIG_PATH
        with open(CONFIG_PATH, 'r') as file:
            all_configs = yaml.safe_load(file)
            if all_configs is None or hyperparameter_set not in all_configs:
                raise ValueError(f"CRITICAL ERROR: '{hyperparameter_set}' was not found in hyperparameters.yml!")
